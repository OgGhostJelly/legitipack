# Called by mapimation/advancement/wand_secondary_used.json when the right-click detection gets activated.
# Has code for a cooldown so the right click can't be spammed

advancement revoke @s only mapimation:wand_secondary_used
...
execute if score @s mapimation_wand_cooldown matches ..0 as @n[type=minecraft:marker,tag=MapimationWandMarker] at @s positioned ~ ~1.8 ~ run function mapimation:wand/secondary_used_inner
